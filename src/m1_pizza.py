...
def run_test_generate_points_on_circle():
    ...
    # Test 2:
    expected = [rg.Point(108.0, 10.0), rg.Point(100.0, 18.0), rg.Point(92.0, 10.0), rg.Point(100.0, 2.0)]
    circle = rg.Circle(rg.Point(100, 10), 8)
...
def run_test_draw_points_on_circle():
    ...
    # Test 4:
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  4 orange dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(150, 150), 50)
    draw_points_on_circle(window, circle, 4, 'orange')
    window.close_on_mouse_click()
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    locations = generate_points_on_circle(circle,number_of_points)

    for k in range(len(locations)):
        smallCirlce = rg.Circle(locations[k],10)
        smallCirlce.fill_color = color
        circle.attach_to(window)
        smallCirlce.attach_to(window)
        locations[k].attach_to(window)
    window.render()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    locations = generate_points_on_circle(circle,number_of_slices)

    circle.attach_to(window)

    for k in range(len(locations)):
        line = rg.Line(circle.center,locations[k])
        line.color = color
...
def run_test_polygon():
    ...
    # Test 3:
    title = ('POLYGON test 3: 8 segments with thick ORANGE lines')
    window = rg.RoseWindow(550, 400, title)

    circle = rg.Circle(rg.Point(150, 150), 80)
    circle.outline_thickness = 3
    polygon(window, circle, 8, 'orange', 12)
    window.continue_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    locations = generate_points_on_circle(circle,number_of_segments)

    circle.attach_to(window)

    for k in range(number_of_segments):
        line = rg.Line(locations[k],locations[(k+1)%number_of_segments])
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
    window.render()
...
def run_test_fancy_polygon():
    ...
    title = ('FANCY POLYGON test 4:  200 black green lines on a yellow circle,'
             + ' hops = 8.')
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(150, 150), 150)
    circle.fill_color = 'yellow'
    fancy_polygon(window, circle, 50, 8, 'black', 2)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    locations = generate_points_on_circle(circle,number_of_lines)

    circle.attach_to(window)

    for k in range(number_of_lines):
        line = rg.Line(locations[k],locations[(k+hops_to_next_point)%number_of_lines])
        line.color = color
        line.thickness = thickness
        line.arrow = 'last'
        line.attach_to(window)
